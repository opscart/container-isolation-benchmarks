...

{
        ...
        for test in baseline moderate aggressive control; do
            ...
            if [ -f "$TEST_FILE" ]; then
                ...
                if [ -n "$NR_PERIODS" ] && [ "$NR_PERIODS" -gt 0 ]; then
                    THROTTLE_PCT=$(echo "scale=1; ($NR_THROTTLED / $NR_PERIODS) * 100" | bc -l)
    ...
    if [ -d "$RESULTS_DIR/03-network-latency" ]; then
        ...
        if [ -f "$LOOPBACK_FILE" ] && [ -f "$VETH_FILE" ]; then
            ...
            if [ -n "$LOOPBACK_LAT" ] && [ -n "$VETH_LAT" ]; then
                ...
                OVERHEAD_PCT=$(echo "scale=1; ($OVERHEAD / $LOOPBACK_LAT) * 100" | bc -l)
                ...
                if (( $(echo "$OVERHEAD_ABS < 1" | bc -l) )); then
                    ...
                    echo "  Kernel $(uname -r) has highly optimized veth"
    ...
    echo ""
    echo "3. Network Latency (<1μs on 6.14):"
    ...
    echo "Key Takeaways:"
    echo ""
    echo "  ✅ Container networking overhead solved (kernel 6.14)"
